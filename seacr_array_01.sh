...
############################### MERGED BIOLOGICAL REPLCIATE PROCESSING ###############################
...
# OUTPUT_DIR="../results/merged/peaks/seacr_$VAR1\_$VAR2\_S9_W5"
OUTPUT_DIR="../results/merged/peaks/seacr_$VAR1\_$VAR2\_S0_W4"
# OUTPUT_DIR="../results/unmerged/peaks/seacr_{$VAR1}_{$VAR2}"
...
# Extract components from the filename using regex
        if [[ $SAMPLE_NAME =~ ^(LIB.*)_(S[0-9]+|W[0-9]+|WX|SX)_(NS|S)_(AR|BRG|K27A|K27M|K4|IgG)_.*$ ]]; then
        ...
        if [ -r "$IG_CONTROL_FILE" ]; then
            ...
            # Uncomment the following line to run the SEACR command (IgG-based analysis)
