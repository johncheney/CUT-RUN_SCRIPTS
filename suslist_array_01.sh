...
# INPUT_DIR="../results/bowtie2_l/sam" # first array dir
...
# INPUT_DIR="../results/suspect_list_filter_l/sorted_bam" # fourth array dir
INPUT_DIR="../results/suspect_list_filter_l/bai" # fifth array dir

SL_OUTPUT_DIR="../results/suspect_list_filter_l"

mkdir -p $SL_OUTPUT_DIR
...
mkdir -p $SL_OUTPUT_DIR/{bam,Nordin,merge,sorted,sorted_bam,bai,bigwig}

# Iterate over SAM files in INPUT_DIR
# for SAM_FILE in "$INPUT_DIR"/*.sam; do
#     if [ -r "$SAM_FILE" ]; then
        # SAMPLE_NAME=$(basename "$SAM_FILE" .sam)
        SAMPLE_NAME=$(basename "$1" ) # array style line here
	...
	# samtools sort -n -o $SL_OUTPUT_DIR/sorted/${SAMPLE_NAME}_sorted.sam $INPUT_DIR/${SAMPLE_NAME} #correct syntax for an array style submision
# else
...
#     if [ -r "$SAM_FILE" ]; then
#         SAMPLE_NAME=$(basename "$SAM_FILE" .sam) # for loop syntax
        # SAMPLE_NAME=$(basename "$1" ) # array style line here
#         samtools view -hbS -F 0x04 $SL_OUTPUT_DIR/sorted/${SAMPLE_NAME}.sam > $SL_OUTPUT_DIR/bam/${SAMPLE_NAME}_mapped.bam
        # samtools view -hbS -F 0x04 $INPUT_DIR/${SAMPLE_NAME} > $SL_OUTPUT_DIR/bam/${SAMPLE_NAME}_mapped.bam # array optimized
# else
#         echo "Error: No SAM files found in $INPUT_DIR"
#         exit 1                                                                                                                                           
#     fi      
...
#         SAMPLE_NAME=$(basename "$BAM_FILE" .bam)
        ...
        # # echo "Processing $BAM_FILE with params: $params" # loop syntax
        # echo "Processing $SAMPLE_NAME with params: $params" # array syntax
        
        # # Perform samtools and awk processing
        # # Save intermediate output after the awk command
        # # samtools view -H "$BAM_FILE" > $SL_OUTPUT_DIR/bam/${SAMPLE_NAME}_header.sam # loop syntax
        # # samtools view -H "$INPUT_DIR/$SAMPLE_NAME" > $SL_OUTPUT_DIR/bam/${SAMPLE_NAME}_header.sam # array syntax
        
        # # samtools view -h -q 30 -F 0xF04 "$BAM_FILE" | \ # loop syntax (next line is array syntax)
        # samtools view -h -q 30 -F 0xF04 "$INPUT_DIR/$SAMPLE_NAME" | \
        #     samtools fixmate - - | \
        #     samtools view -f 0x3 - | \
        ...
        #Array Syntax below
        # cat $INPUT_DIR/${SAMPLE_NAME}_header.sam $INPUT_DIR/${SAMPLE_NAME}_filtered.sam | \
        ...
        # actually sort them this time Array Syntax
	    # samtools sort -o $SL_OUTPUT_DIR/sorted_bam/${SAMPLE_NAME} $INPUT_DIR/${SAMPLE_NAME} #correct syntax for an array style submision

        # # Index the sorted BAM file Array syntax
        # samtools index -M $INPUT_DIR/${SAMPLE_NAME} $SL_OUTPUT_DIR/bai/${SAMPLE_NAME}.bai # move bams over
        
        # # Index the sorted BAM file loop syntax
        # samtools index $SL_OUTPUT_DIR/bam/${SAMPLE_NAME}_sorted.bam

        #making bigwigs array syntax
        bamCoverage -b $INPUT_DIR/${SAMPLE_NAME} -o $SL_OUTPUT_DIR/bigwig/${SAMPLE_NAME}.bigwig \
        --binSize 5 \
        --normalizeUsing BPM \
        --smoothLength 60 \
        --extendReads \
